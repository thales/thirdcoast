class Admin::PageTemplatesController < Admin::ApplicationController
  ...
  def index
    @page_templates = PageTemplate.find(:all, :order => 'title')
  ...
  def create
    ...
    respond_to do |format|
      if @page_template.save
        flash[:notice] = 'page_template was successfully created.'
        format.html { redirect_to(admin_page_template_path(@page_template)) }
  ...
  def update
    ...
    respond_to do |format|
      if @page_template.update_attributes(params[:page_template])
        flash[:notice] = 'page_template was successfully updated.'
        format.html { redirect_to(admin_page_template_path(@page_template)) }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(admin_page_templates_url) }
      format.xml  { head :ok }
    end
  end
end
