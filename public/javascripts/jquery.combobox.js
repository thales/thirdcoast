...
jQuery.fn.combobox = function(options)
{
	...
	return this.each(
		function()
		{
			...
			var _containerDefaultStyle = "border-left: solid 1px #786f63;border-top: solid 1px #786f63;border-right: solid 1px #786f63;border-bottom: solid 1px #786f63;";
			...
			function buildDropDownItem(childJQuery)
			{
				...
				if (childJQuery.is('option'))
				{
					...
					if (dataItemClass)
					{						
						dataItemHTML = String.format("<li rel='"+dataItemValue+"' style='{0}' class='{1}'>{2}</li>", dataItemStyle, dataItemClass, dataItemText);
					}
					else
					{
						dataItemHTML = String.format("<li rel='"+dataItemValue+"' style='{0}'>{1}</li>", dataItemStyle, dataItemText);
					}
				...
				else
				{
					...
					if (dataItemClass)
					{						
						dataItemHTML = String.format("<li rel='"+dataItemValue+"'><span style='{0}' class='{1}'>{2}</span></li>", dataItemStyle, dataItemClass, dataItemText);
					}
					else
					{
						dataItemHTML = String.format("<li rel='"+dataItemValue+"'><span style='{0}'>{1}</span></li>", dataItemStyle, dataItemText);
					}
				}
        ...
        // jQuery("*", _dropDownListJQuery).not("ul").not("span").not("[@dataType='optgroup']").each(
				jQuery("*", _dropDownListJQuery).not("ul").not("span").each(
					function()
					{
          ...
          _lastItemSelectedJQuery = jQuery("li[rel='"+_lastValue+"']", _dropDownListJQuery);
        ...
        //   elementJQuery = $( elementJQuery[0] )
        // }
        // alert(elementJQuery.length)
        if (elementJQuery)
        {
			...
			function toggleDropDownList(isShown)
			{
				if (isShown)
				{
					if (_dropDownListJQuery.is(":hidden"))
					{
						...
						toggleItemHighlight(_lastItemSelectedJQuery, true); //had to switch of because of jquery 1.4 incompatibility
